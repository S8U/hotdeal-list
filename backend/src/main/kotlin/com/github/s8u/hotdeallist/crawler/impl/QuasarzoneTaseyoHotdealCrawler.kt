...
@Component
class QuasarzoneTaseyoHotdealCrawler : HotdealCrawler {

    override fun getPlatformType(): PlatformType {
        return PlatformType.QUASARZONE_TASEYO
    }

    /**
     * @param url https://quasarzone.com//bbs/qb_tsy/views/{postId}
     */
    override fun getPlatformPostId(url: String): String? {
        if (!url.startsWith("https://quasarzone.com//bbs/qb_tsy/views/")) {
            return null
        }
    ...
    override fun crawlList(page: Int): HotdealCrawlListDto {
        val document = Jsoup.connect("https://quasarzone.com/bbs/qb_tsy?page=${page}").get()
    ...
    override fun crawlDetail(url: String): HotdealCrawlDetailDto {
        ...
        try {
            ...
            val title = document.select("#content h1.title").clone().apply { select("span").remove() }.text().trim()
            val category = null
            ...
            val isEnded = document.selectFirst("#content .title .done") != null
